<?php 
	...
	if(socialink_twitter_is_connected($user->getGUID())){
		...
		$twitter_screen_name = get_plugin_usersetting("twitter_screen_name", $user->getGUID(), "socialink");
...
//			echo "&nbsp;" . elgg_view("input/pulldown", array("internalname" => "params[thewire_post_twitter]", "options_values" => $yesno_options_values, "value" => $usersettings->thewire_post_twitter));
//			echo "</div>";
//		}
		...
		if($fields = socialink_get_configured_network_fields("twitter")){
			...
			echo "&nbsp;" . elgg_view("input/pulldown", array("internalname" => "params[twitter_sync_allow]", "options_values" => array_reverse($noyes_options_values), "value" => $usersettings->twitter_sync_allow, "js" => "onchange='socialink_toggle_network_configure(this, \"twitter\");'"));
			...
			foreach($fields as $setting_name => $profile_field){
				...
				echo "<td>" . elgg_view("input/pulldown", array("internalname" => "params[" . $setting . "]", "options_values" => array_reverse($noyes_options_values, true), "value" => $usersettings->$setting)) . "</td>";
				echo "<tr>";
			}
		...
		if(is_plugin_enabled("thewire")){
			...
			if(($twitter_in = get_plugin_setting("twitter_allow_in", "socialink")) && ($twitter_in != "no")){
				echo "<br />";
				echo "<div>";
				echo elgg_echo("socialink:usersettings:twitter:in");
				echo "&nbsp;" . elgg_view("input/pulldown", array("internalname" => "params[twitter_in]", "value" => $usersettings->twitter_in, "options_values" => $noyes_options_values));
				echo "<br />";
			...
			echo elgg_view("input/hidden", array("internalname" => "params[twitter_in]", "value" => "no"));
		}
		
	} else {
		$link_begin = "<a href='" . $vars["url"] . "pg/socialink/forward/twitter/authorize' target='_self'>";
		$link_end = "</a>";
