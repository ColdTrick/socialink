<?php 
	...
	if($keys = socialink_facebook_is_connected($user->getGUID())){
		...
		echo "<div>" . sprintf(elgg_echo("socialink:usersettings:facebook:remove"), $link_begin, $link_end) . "</div>";
		
		// check for the wire
//		if(is_plugin_enabled("thewire")){
//			echo "<div>";
//			echo elgg_echo("socialink:usersettings:facebook:thewire");
//			echo "&nbsp;" . elgg_view("input/pulldown", array("internalname" => "params[thewire_post_facebook]", "options_values" => $yesno_options_values, "value" => $usersettings->thewire_post_facebook));
//			echo "</div>";
//		}
		...
		if($fields = socialink_get_configured_network_fields("facebook")){
			...
			echo "&nbsp;" . elgg_view("input/pulldown", array("internalname" => "params[facebook_sync_allow]", "options_values" => array_reverse($yesno_options_values), "value" => $usersettings->facebook_sync_allow, "js" => "onchange='socialink_toggle_network_configure(this, \"facebook\");'"));
			...
			echo "<table id='socialink_facebook_sync_fields'>";
			echo "<tr>";
		...
		$link_begin = "<a href='" . $vars["url"] . "pg/socialink/forward/facebook/authorize' target='_self'>";
		$link_end = "</a>";
		
		echo sprintf(elgg_echo("socialink:usersettings:facebook:not_connected"), $link_begin, $link_end);
	}
	echo "</div>";
