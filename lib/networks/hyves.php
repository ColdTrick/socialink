<?php
/**
 * All Hyves releated helper functions are bundeld here
 */

define("SOCIALINK_HYVES_API_VERSION", "2.1");

/**
 * Get the Hyves API opbject
 *
 * @param array $keys API keys
 *
 * @return bool|HyvesProxy
 */
...
/**
 * Check if the user is connected to a Hyves account
 *
...
/**
 * Get the URL to authorize Hyves
 *
...
/**
 * Get a Hyves access token
 *
 * @param string $oauth_verifier the API state
 *
 * @return bool|GenusOAuthRequestToken
 */
...
/**
 * Get profile information from Hyves
 *
...
/**
 * Update the Elgg profile with Hyves data
 *
...
/**
 * Update the user profile with the Hyves profile image
 *
...
/**
 * Create a user based on Hyves information
 *
 * @param GenusOAuthAccessToken $token Hyves access token
...
/**
 * Is the connection to Hyves still valid
 *
 * @param int $user_guid the user_guid to check
 *
 * @return bool
...
function socialink_hyves_validate_connection($user_guid = 0) {
	...
	if ($keys = socialink_hyves_is_connected($user_guid)) {
		if ($api = socialink_hyves_get_api_object($keys)) {
			...
			try {
				$api->doMethod($method, $params, $token);
			} catch (Exception $e) {
				$result = false;
			}
		}
